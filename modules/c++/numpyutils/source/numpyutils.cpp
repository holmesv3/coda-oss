...
CODA_OSS_disable_warning(-Wold-style-cast) CODA_OSS_disable_warning(-Wshadow)
                CODA_OSS_disable_warning(-Wsuggest-override) CODA_OSS_disable_warning(-Wzero-as-null-pointer-constant)
#endif
...
void verifyArray(PyObject* pyObject)
{
    ...
    {
        throw except::Exception(Ctxt("Invalid data type (expected numpy array)"));
    }
}
...
const npy_intp* getDimensions(PyObject* pyArrayObject)
{
    ...
    if (ndims != 2)
    {
        throw except::Exception(Ctxt("Numpy array has dimensions different than 2"));
    }
...
void verifyObjectsAreOfSameDimensions(PyObject* lhs, PyObject* rhs)
{
    ...
    {
        throw except::Exception(Ctxt("Numpy arrays are of differing dimensions"));
    }
}
...
void createOrVerify(PyObject*& pyObject, int typeNum, const types::RowCol<size_t>& dims)
{
    ...
    else
    {
        ...
        {
            throw except::Exception(Ctxt("Desired array does not match required row, cols"));
        }
    }
}
...
PyObject* toNumpyArray(size_t numRows, size_t numColumns, int typenum, const void* data)
{
    ...
    PyObject* wrappedArray =
            PyArray_SimpleNewFromData(nDims, dimensions, typenum, const_cast<void*>(data));
    PyObject* copy = PyArray_NewCopy(reinterpret_cast<PyArrayObject*>(wrappedArray), NPY_CORDER);
...
PyObject* toNumpyArray(size_t numColumns, int typenum, const std::vector<void*>& data)
{
    ...
    for (size_t ii = 0; ii < numRows; ++ii)
    {
        ...
        PyObject* row = PyArray_SimpleNewFromData(1, dimensions, typenum, data[ii]);
...
void prepareInputAndOutputArray(PyObject* pyInObject,
                                ...
                                int outputTypeNum)
{
    prepareInputAndOutputArray(
            pyInObject, pyOutObject, inputTypeNum, outputTypeNum, getDimensionsRC(pyInObject));
}
