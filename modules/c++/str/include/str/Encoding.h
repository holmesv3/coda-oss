...
inline auto cast(const TChar* s)
{
    ...
    static_assert(sizeof(*retval) == sizeof(*s),
                  "sizeof(*TReturn) != sizeof(*TChar)");
    return retval;
}
}
...
CODA_OSS_API coda_oss::u8string to_u8string(str::W1252string::const_pointer,
                                            size_t);
CODA_OSS_API coda_oss::u8string to_u8string(std::u16string::const_pointer,
                                            size_t);
CODA_OSS_API coda_oss::u8string to_u8string(std::u32string::const_pointer,
                                            size_t);
inline coda_oss::u8string to_u8string(coda_oss::u8string::const_pointer p,
                                      size_t sz)
{
...
CODA_OSS_API std::u16string to_u16string(coda_oss::u8string::const_pointer,
                                         size_t);
CODA_OSS_API std::u16string to_u16string(str::W1252string::const_pointer,
                                         size_t);
...
CODA_OSS_API std::u32string to_u32string(coda_oss::u8string::const_pointer,
                                         size_t);
CODA_OSS_API std::u32string to_u32string(str::W1252string::const_pointer,
                                         size_t);
...
// encoding on Windows.
CODA_OSS_API str::W1252string to_w1252string(
        coda_oss::u8string::const_pointer p, size_t sz);
...
{
#if _WIN32
    const auto p = str::c_str<str::W1252string>(
            s);  // std::string is Windows-1252 on Windows
#else
    const auto p = str::c_str<coda_oss::u8string>(
            s);  // assume std::string is UTF-8 on any non-Windows platform
#endif
...
#else
    return str::c_str<std::u32string>(
            s);  // assume std::wstring is UTF-32 on any non-Windows platform
#endif
}
}
inline auto u8FromNative(
        const std::wstring& s)  // platform determines UTF16 or UTF-32 input
{
...
CODA_OSS_API std::string to_string(
        const std::wstring&);  // input is UTF-16 or UTF-32 depending on the
                               // platform
CODA_OSS_API std::wstring to_wstring(
        const std::string&);  // platform determines Windows-1252 or UTF-8 input
                              // and output encoding
CODA_OSS_API std::wstring to_wstring(
        const coda_oss::u8string&);  // platform determines UTF-16 or UTF-32
                                     // output encoding
}
namespace testing
{
...
CODA_OSS_API std::wstring to_wstring(
        const str::W1252string&);  // platform determines UTF-16 or UTF-32
                                   // output encoding
}

inline std::string to_native(
        const coda_oss::u8string&
                s)  // cf., std::filesystem::native(),
