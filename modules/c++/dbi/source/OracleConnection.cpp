                                    ...
                                    unsigned int port)
{
    if (port == 0)
        port = 1521;
    int rc = OCILogon(mEnvHandle,
                      mErrorHandle,
                      &mContextHandle,
                      (const OraText*)user.c_str(),
                      user.length(),
                      (const OraText*)pass.c_str(),
                      pass.length(),
                      (const OraText*)database.c_str(),
                      database.length());
    if (rc != 0)
        return false;
    return true;
...
const std::string dbi::OracleConnection::getLastErrorMessage()
{
    ...
    memset(errbuf, 0, 100);
    int errcode;
    OCIErrorGet((dvoid*)mErrorHandle,
                (ub4)1,
                (text*)nullptr,
                &errcode,
                (OraText*)errbuf,
...
dbi::Row dbi::OracleResultSet::fetchRow()
{
    ...
    if (result == OCI_SUCCESS)
    {
        for (int i = 0; i < (int)count; i++)
        {
            ...
            {
                text* temp;
                result = OCIAttrGet(param_handle,
                                    OCI_DTYPE_PARAM,
                                    &temp,
                                    &name_len,
                                    OCI_ATTR_NAME,
            ...
            if (result == OCI_SUCCESS)
            {
                ...
                OCIDefine* defineHandle = nullptr;
                result = OCIDefineByPos(mSQLHandle,
                                        &defineHandle,
                                        mErrorHandle,
                                        i + 1,
                                        fields[i].value,
        ...
        if (result == OCI_SUCCESS || result == OCI_NO_DATA || result == OCI_SUCCESS_WITH_INFO)
        {
            for (int i = 0; i < (int)count; i++)
            {
                ...
                if (fields[i].extType == SQLT_ODT)
                {
                    ...
                    OCIDateToText(mErrorHandle,
                                  (const OCIDate*)fields[i].value,
                                  nullptr,
                                  0,
                                  nullptr,
                                  0,
                ...
                row.addField((const char*)fields[i].name,
                             fields[i].type,
                             fields[i].fieldSize,
                             fields[i].value);
