...
struct Assert final
{
    static void FailOnCondition(
            bool condition,
            const unsigned short* message,
            const Microsoft::VisualStudio::CppUnitTestFramework::__LineInfo*
                    pLineInfo);
    static std::wstring GetAssertMessage(bool equality,
...
#define CODA_OSS_equals_message_(expected, actual, message)          \
    reinterpret_cast<const unsigned short*>(                         \
            test::Assert::GetAssertMessage(true,                     \
...
#define CODA_OSS_not_equals_message_(notExpected, actual, message)      \
    reinterpret_cast<const unsigned short*>(                            \
            test::Assert::GetAssertMessage(false,                       \
                                           test::toString(notExpected), \
                                           test::toString(actual),      \
                                           message)                     \
                    .c_str())
...
inline void diePrintf_eq(
        ...
        const Microsoft::VisualStudio::CppUnitTestFramework::__LineInfo*
                pLineInfo = NULL)
{
    Assert::FailOnCondition(expected == actual,
                            CODA_OSS_equals_message_(expected, actual, message),
                            pLineInfo);
}

// see Assert::AreNotEqual<>
template <typename TExpected, typename TActual>
inline void diePrintf_ne(
        const TExpected& notExpected,
        const TActual& actual,
        const wchar_t* message = NULL,
        const Microsoft::VisualStudio::CppUnitTestFramework::__LineInfo*
                pLineInfo = NULL)
{
    Assert::FailOnCondition(notExpected != actual,
                            CODA_OSS_not_equals_message_(notExpected,
                                                         actual,
                                                         message),
                            pLineInfo);
}
...
#define CODA_OSS_test_diePrintf_eq_(X1, X2) \
    testName, ::test::diePrintf_eq(X1, X2)
#define CODA_OSS_test_diePrintf_not_eq_(X1, X2) \
    testName, ::test::diePrintf_ne(X1, X2)
...
#define TEST_ASSERT_ALMOST_EQ_EPS(X1, X2, EPS)                                \
    {                                                                         \
        (void)testName;                                                       \
        Microsoft::VisualStudio::CppUnitTestFramework::Assert::AreEqual(X1,   \
                                                                        X2,   \
                                                                        EPS); \
        Microsoft::VisualStudio::CppUnitTestFramework::Assert::AreEqual(X2,   \
                                                                        X1,   \
                                                                        EPS); \
    }
namespace test
...
inline void assert_almost_eq(const std::string& testName,
                             long double X1,
                             long double X2)
{
    assert_almost_eq(testName,
...
#define TEST_ASSERT_EQ_MSG(msg, X1, X2)                             \
    testName,                                                       \
            Microsoft::VisualStudio::CppUnitTestFramework::Logger:: \
                    WriteMessage(msg.c_str());                      \
...
#define TEST_SPECIFIC_EXCEPTION(X, Y)                               \
    testName,                                                       \
            Microsoft::VisualStudio::CppUnitTestFramework::Assert:: \
                    ExpectException<Y>([&]() { (X); })
