...
TEST_CASE(GetNumThreadsThatWillBeUsedTest)
{
    ...
    TEST_ASSERT_EQ(planner4.getNumThreadsThatWillBeUsed(),
    ...
    TEST_ASSERT_EQ(planner5.getNumThreadsThatWillBeUsed(),
                   static_cast<size_t>(100));

    // Have more elements than threads - only the first NUM_ELEMENTS threads
    // are needed
    const mt::ThreadPlanner planner6(NUM_ELEMENTS, 101);
    TEST_ASSERT_EQ(planner6.getNumThreadsThatWillBeUsed(),
                   static_cast<size_t>(100));

    const mt::ThreadPlanner planner7(NUM_ELEMENTS, 9999);
    TEST_ASSERT_EQ(planner7.getNumThreadsThatWillBeUsed(),
                   static_cast<size_t>(100));
}

TEST_MAIN(TEST_CHECK(GetThreadInfoTest);
